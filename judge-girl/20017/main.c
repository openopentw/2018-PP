...
#define MAXGPU 10
#define MAXK 1024
#define MAXGLOBAL 1024
#define LOCALSIZE 1024
#define SEQSIZE 64
/* #define MAXGLOBAL 65536 */
/* #define LOCALSIZE 1024 */
/* #define SEQSIZE 1 */
...
int main (void) {
	...
	while (scanf("%d%d%d", &N, &key1, &key2) == 3) {
		...
		size_t globalThreads[] = {(size_t)(N / LOCALSIZE / SEQSIZE) * LOCALSIZE};
		...
		uint32_t sum = 0;
		if (N % (LOCALSIZE * SEQSIZE) > 0) {
			for (int i = globalThreads[0] * SEQSIZE; i < N; ++i) {
		...
		clEnqueueReadBuffer(commandQueue, bufferAns, CL_TRUE, 0,
							(N / LOCALSIZE / SEQSIZE) * sizeof(cl_uint), ans, 0, NULL, NULL);
		int endId = N / LOCALSIZE / SEQSIZE;
